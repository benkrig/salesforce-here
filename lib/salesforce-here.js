	...
	*/
	...
	testfun : function(params, callback)
	{
		...
		conn.login(params.username, params.password+params.token, function(err, CONNECTION_INFORMATION)
		{
			...
			conn.metadata.list(lstquery,function(err,res)
			{
				/*
					FUCK THIS BULLSHIT API
					FUCK THE RETARDS WHO MADE CERTAIN METADATA TYPES BE SELECTABLE THROUGH ONLY FUCKING FOLDERS
				*/
				var emailtemplatexmlnames = [];
				if (err) 
				{ 
				...
				{
					folders.push(res[c].fullName);
				};

				var query = [];
				folders.forEach(function(item)
				{
					query.push({type: 'EmailTemplate', folder: item});
				});
				...
				while(query.length > 0) 
				{
  					...
  					conn.metadata.list(chunk, function(err,res)
					{
						if(err) return console.log(err);
						
						res.forEach(function(item)
						{
							emailtemplatexmlnames.push(item.fullName);
						});

						if(done = 1)
						{
							var rt = emailtemplatexmlnames[(emailtemplatexmlnames.length-1)];
							console.info(emailtemplatexmlnames);
							return callback(emailtemplatexmlnames);
						}
					});
				}
			});
