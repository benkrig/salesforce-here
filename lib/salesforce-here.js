		...
		s3.getObject(params, function(err, data) 
		{
 			if (err) 
  			...
  			else 
  			{
  				...
  				var arrayBuffer = new ArrayBuffer(dataBuffer.length);
				var array = new Uint8Array(arrayBuffer);

				var ab = new ArrayBuffer(dataBuffer.length);
    			var view = new Uint8Array(ab);
    			var count = 0;

    			async.whilst(
				    function () { return count < dataBuffer.length; },
				    function (wcallback) 
				    {
				        count++;
        				view[count] = dataBuffer[count];
        				wcallback();
				    },
				    function (err) 
				    {
