...
var getFolders = function(response, callback)
{
	...
	{
		folders.push(response.fullName);
		return callback(folders);
	}
};
	...
	newPackageDotXML: function(metadata, callback)
	{
		if(metadata.xmlnames.length > 0)
		{
			...
			while(done == 0)
			{
				...
				{
					if(count < metadata.xmlnames.length)
					{
						if(metadata.xmlnames[count] == 'EmailTemplate')
						{
							module.exports.testfun(metadata, function(membernames)
							{
								package.types.push(
								{
									'members' : membernames,
		...
		conn.login(params.username, params.password+params.token, function(err, CONNECTION_INFORMATION)
		{
			...
			conn.metadata.list(lstquery,function(err,res)
			{
				...
				*/
				...
				getFolders(res, function(folders)
				{
					getFileNames(conn, folders, function(filenames)
					{
						return callback(filenames);
