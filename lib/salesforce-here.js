...
var getFolders = function(response, callback)
{
	...
	if(Array.isArray(response))
	{
		...
		async.each(response, function (item, cback)
		{ 
        	console.log('*****');
        	folders.names.push(item.fullName);
        	cback();
...
var getFileNames = function(conn, folders, callback)
{
	...
	async.each(folders.names, function (item, cback)
	{ 
       	console.log('***** ' + item);
									...
									conn.metadata.list(reportq,function(err,eres2)
									{
										...
										getFolders(eres2, function(efolders2)
										{
											getFileNames(conn, efolders2, function(efilenames2)
											{
												...
												conn.metadata.list(dashboardq, function(err, dashboardresponse)
												{
													console.log(dashboardresponse + "dashres");
													if(err)
													{
														return console.log(err);
													}

													getFolders(dashboardresponse, function(dashboardfolders)
													{
