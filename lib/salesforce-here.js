	...
	testfun : function(params, callback)
	{
		...
		conn.login(params.username, params.password+params.token, function(err, CONNECTION_INFORMATION)
		{
			...
			*/
			...
			async.each(qs, function(que, callback) 
			{
				console.log(que);
				conn.metadata.list(que,function(err,res)
				{
					//type not found
					if (err) 
					{ 
					...
					getFolders(res, function(resfolder)
					{
						getFileNames(conn, resfolder, function(resnames)
						{
							if(que.indexOf("EmailFolder") > -1)
							{
								fnames.emailnames = resnames;
							}
							if(params.xmlnames.indexOf("DocumentFolder") > -1)
							{								
								fnames.documentnames = resnames;
							}
							if(params.xmlnames.indexOf("DashboardFolder") > -1)
							{
								fnames.dashboardnames = resnames;
							}
							if(params.xmlnames.indexOf("ReportFolder") > -1)
							{
								fnames.reportnames = resnames;
							}
							callback();
						});
					});
			...
			/*conn.metadata.list(qemailtemplate,function(err,emailres)
			{
				//type not found
				if (err) 
				{ 
				...
				getFolders(emailres, function(emailfolders)
				{
					getFileNames(conn, emailfolders, function(emailfilenames)
					{
						fnames.emailnames = emailfilenames;
